...
**Статус:** выполняется. Фиксированный simulation timestep и интерполяция render state работают; следующий этап — коллизии мира и движение капсулы.
...
**Последнее обновление:** 21 июля 2026 — п. 29 выполнен: C++ runtime выполняет simulation с фиксированным шагом 1/60 s, ограничивает catch-up и предоставляет alpha для интерполяции render state; десятисекундный regression даёт 600 одинаковых ticks и сопоставимое состояние при 30/60/120 Hz.
