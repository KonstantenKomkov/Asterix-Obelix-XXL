...
|---:|---|---|---|---|---|
| 83 | **Провести сквозной runtime-аудит и правильно привязать все анимации игры:** сопоставить каждое фактически достижимое действие, состояние, событие и переход с authored clip оригинала, устранить общие состояния, ошибочные semantic aliases и fallback, скрывающие разные анимации | M4 fidelity | P0 | XL | После п. 62–69 и фактических замечаний по бегу/двойному прыжку; охват обязателен для Астерикса, Обеликса, Идефикса, всех врагов, боссов, NPC и существ, интерактивных объектов и механизмов, environment/particle/material/water FX, UI, scripted-сцен и cinematics. Для каждого actor/object/profile требуется полный список достижимых runtime-состояний, переходов и событий; для управляемых героев отдельно покрыть idle, старт/остановку и направления движения, бег/ходьбу, прыжки и воздушные варианты, падение/приземление, traversal/swim, атаки/combo, урон, смерть и interactions. Обязательны покадровая сверка каждого binding с локальным оригиналом и соответствующим исходным dictionary/slot/event, отдельное data-driven runtime-состояние для каждого визуально различимого действия без clip IDs в renderer, корректные loop/clamp, cadence, blend, root motion и animation events; машинная проверка, что каждый из 345 clips и 518 slots имеет правильный контекст, каждый достижимый runtime-переход разрешает ровно один совместимый binding, а ни один clip не подменяет другое действие только ради формальной полноты graph. Fresh ASTPAK, cold-start representative sequences для каждой группы, scripted/cinematic сценарии и зафиксированная visual acceptance должны подтверждать результат без ручных диагностических overrides и silent static fallback |
...
**Последнее обновление:** 23 июля 2026 — область п. 83 расширена с одного Астерикса до всех анимаций игры: управляемых героев, врагов, боссов, NPC, существ, объектов, механизмов, environment/particle/material/water FX, UI и scripted/cinematic-сцен. Предыдущие проверки формальной полноты каталога и graph не гарантировали правильное визуальное соответствие runtime-состояний authored clips, поэтому задача требует покадрово принять все 345 clips / 518 slots на фактически достижимых runtime paths, fresh ASTPAK и representative cold-start/scenario sequences без semantic-подмен и silent fallback.
