...
**Последнее обновление:** 22 июля 2026 — п. 59 выполнен: idle/run palette плавно смешиваются, фаза run-клипа следует фактической скорости капсулы, а модель сохраняет направление движения; fixed-tick и visual pose regressions покрывают оба перехода.
