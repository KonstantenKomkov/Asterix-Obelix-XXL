...
| 57 | Реализовать и проверить двойной прыжок | Gameplay | P1 | M | Второе нажатие прыжка в воздухе запускает ровно один дополнительный импульс; третий прыжок невозможен; доступ к двойному прыжку восстанавливается только после подтверждённого приземления; переходы анимаций покрыты тестом |
| 58 | Добавить управляемую высоту прыжка в зависимости от длительности удержания кнопки | Gameplay | P1 | M | Короткое нажатие даёт заметно меньшую высоту, удержание до заданного лимита — полную; отпускание кнопки обрезает восходящую фазу без рывка; минимальная/максимальная высота и поведение обоих прыжков детерминированы и покрыты тестами |
| 59 | Восстановить locomotion-анимации Астерикса при движении | Gameplay bugfix | P0 | M | После п. 51 и 56; при ненулевой горизонтальной скорости state machine включает подходящий walk/run clip вместо idle, направление/скорость playback согласованы с движением капсулы, возврат в idle не дёргает позу; переходы idle↔move покрыты native и visual regression-тестами |
| 60 | Откалибровать скорость движения Астерикса по эталонному прохождению оригинала | Gameplay fidelity | P0 | M | После п. 5 и 56; на зафиксированном прямом маршруте измерены скорость и время прохождения в оригинале, `run_speed`, acceleration/deceleration и диагональная нормализация настроены в согласованном масштабе мира; расхождение дистанции/времени укладывается в документированный допуск и покрыто fixed-tick regression-тестом |
...
- [ ] П. 59 — locomotion-анимации при движении
...
**Последнее обновление:** 22 июля 2026 — добавлены п. 59–61 по результату проверки движения: locomotion-анимации, калибровка скорости по оригиналу и подключение gameplay-камеры к позиции капсулы.
