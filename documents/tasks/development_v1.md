...
| 82 | **Заменить красный enemy placeholder полноценным исходным персонажем:** установить по level hooks, spawn/actor bindings и эталонному прохождению, какой персонаж должен находиться на месте `enemyMarker` object ID `900002`, затем подключить его полную render/animation composition | Gameplay fidelity после M4 | P0 | L | Не считать маркер Обеликсом без исходного evidence; связать runtime fighter/enemy ID с подтверждёнными skin atomic, skeleton, материалами, текстурами и animation graph из свежего ASTPAK; модель следует capsule transform, facing и состояниям idle/move/attack/hurt/death, hitbox/hurtbox остаются синхронны; красный `AsterixWriteMarker` недоступен в обычном gameplay и может существовать только как явно включаемый debug overlay; отсутствие/неоднозначность binding выдаёт диагностическую ошибку вместо геометрического placeholder; post-build audit и cold-start/combat visual regressions подтверждают правильного персонажа, масштаб, положение на земле, ориентацию, материалы и смену всех состояний; выполнить совместно с composition-аудитом п. 80 без жёстко заданного одиночного skin ID |
...
- [ ] П. 82 — замена красного enemy placeholder полноценной моделью персонажа
...
**Последнее обновление:** 22 июля 2026 — добавлен п. 82: красный треугольник локализован как явно генерируемый `enemyMarker` object ID `900002`, а не как повреждённый импортированный skin; текущий runtime всегда рисует шесть красных marker-вершин для противника. Задача требует доказать по исходным actor/spawn bindings, какой персонаж должен быть здесь (не предполагая заранее Обеликса), подключить его полную модель и animation graph и оставить marker только в явном debug overlay.
