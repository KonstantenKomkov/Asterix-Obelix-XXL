...
|---:|---|---|---|---|---|
| 83 | **Провести сквозной runtime-аудит и правильно привязать все анимации игры:** сопоставить каждое фактически достижимое действие, состояние, событие и переход с authored clip оригинала, устранить общие состояния, ошибочные semantic aliases и fallback, скрывающие разные анимации | M4 fidelity | P0 | XL | Зонтичная приёмка после п. 84–90. Базовый strict gate уже различает декларативные graph paths и конкретные runtime profiles: из 408 bindings конкретно подключены 8 состояний Астерикса, оставшиеся 400 распределены по группам ниже. Для каждого actor/object/profile требуется полный список достижимых runtime-состояний, переходов и событий; обязательны покадровая сверка каждого binding с локальным оригиналом и соответствующим исходным dictionary/slot/event, отдельное data-driven runtime-состояние для каждого визуально различимого действия без clip IDs в renderer, корректные loop/clamp, cadence, blend, root motion и animation events. Итоговая машинная проверка должна подтвердить 345 clips / 518 slots / 408 concrete runtime bindings / 0 declarative-only bindings, а fresh ASTPAK, cold-start representative sequences всех групп и зафиксированная visual acceptance — работу без diagnostic overrides и silent fallback |
| 84 | **Подключить оставшиеся runtime-анимации Астерикса:** перевести 82 semantic gameplay bindings полного hero graph из declarative-only в конкретные state/event selectors | M4 fidelity | P0 | XL | После базового strict gate п. 83; вместе с уже подключёнными 8 состояниями все 90 gameplay bindings Астерикса должны иметь реальные entry points. Отдельно принять idle variants, старт/остановку и направления, воздушные/landing состояния, combo и contextual attacks, damage/recovery, interactions, ledge/water/swim; каждый достижимый переход выбирает один exact binding, events/root motion/cadence управляются данными. Нужны fixed-tick regressions, fresh ASTPAK и cold-start visual sequences против локального оригинала |
| 85 | **Подключить полный runtime-профиль Обеликса:** перевести 72 gameplay bindings из declarative-only в конкретные state/event selectors | M4 fidelity | P0 | XL | После общей инфраструктуры п. 84; покрыть locomotion/directional transitions, idle variants, attacks/combo/contextual attacks, damage/airborne/recovery/death, interactions, traversal и swim. Все 71 authored clips и повторно используемые slots должны разрешаться точными variants без подмены действия; обязательны deterministic selection, animation events, fresh ASTPAK и representative cold-start/visual acceptance |
| 86 | **Подключить полный runtime-профиль Идефикса:** перевести 28 gameplay bindings из declarative-only в конкретные state/event selectors | M4 fidelity | P0 | L | После общей инфраструктуры п. 84; покрыть locomotion и directional transitions, idle variants, attacks, interactions, death и swim. Повторно используемые clips обязаны оставаться отдельными semantic states там, где различается runtime-событие; нужны deterministic variants, fixed-tick tests, fresh ASTPAK и cold-start/visual acceptance |
| 87 | **Подключить runtime-анимации врагов и лидеров:** перевести 85 gameplay bindings basic Roman и Roman leader из declarative-only в реальные actor instances и state/event selectors | M4 fidelity | P0 | XL | После общей инфраструктуры п. 84 и фактических spawn/profile bindings content runtime; покрыть idle, perception/pursuit/return, attack variants и impact events, stun/knockback, special actions, death/despawn для обычного врага и лидера. Никаких synthetic marker или cross-profile skeleton fallback; нужны authored render compositions, deterministic entity variants, scenario tests, fresh ASTPAK и cold-start combat acceptance |
...
- [ ] П. 84 — оставшиеся 82 runtime bindings Астерикса
- [ ] П. 85 — 72 runtime bindings Обеликса
- [ ] П. 86 — 28 runtime bindings Идефикса
- [ ] П. 87 — 85 runtime bindings врагов и лидеров
