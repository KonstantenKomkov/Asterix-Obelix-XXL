...
**Последнее обновление:** 22 июля 2026 — п. 57 выполнен: реализован ровно один дополнительный воздушный импульс с восстановлением только после подтверждённого приземления; переход `fall → jump` и запрет третьего прыжка покрыты native regression-тестом.
