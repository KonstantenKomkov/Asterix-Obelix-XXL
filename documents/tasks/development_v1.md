...
| 62 | Провести полную семантическую инвентаризацию всех импортированных анимаций | Animation fidelity | P0 | XL | Для всех 345 clips и всех animation dictionaries установлены владелец/skin/costume, действие или событие, loop/one-shot, варианты, допустимые переходы, root motion и animation events; результат хранится как проверяемый машиной каталог со ссылкой на метод подтверждения, без идентификации только по номеру файла; каждый clip имеет однозначный статус и ни один не остаётся молча неразобранным |
| 63 | Реализовать data-driven реестр привязок animation clip → gameplay-событие/действие | Animation architecture | P0 | L | После п. 62; hardcoded номера clips удалены из renderer/state machines, привязки загружаются из versioned manifest по actor, skin/costume, action/event, варианту и контексту; resolver валидирует совместимость skeleton, обязательные states, loop policy, приоритеты, fallback и переходы, а неизвестная или неоднозначная привязка даёт диагностируемую ошибку, а не случайную позу |
...
- [ ] П. 62 — полная семантическая инвентаризация всех анимаций
...
**Последнее обновление:** 22 июля 2026 — добавлены п. 62–69: полная инвентаризация всех 345 анимаций, data-driven bindings, исчерпывающие graphs игрока/персонажей/мира/scripted-сцен, единые animation events и сквозная приёмка с нулём необъяснённых clips; п. 60 уточнён как исправление ускоренной ходьбы на эталонный бег с согласованными gait, cadence и скоростью.
