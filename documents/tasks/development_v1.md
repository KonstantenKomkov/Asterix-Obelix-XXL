...
| 54 | Скопировать иконку из исходного приложения во Flutter-проект и настроить её как иконку macOS-приложения | Presentation | P1 | S | Подтверждены права на использование; иконка добавлена во все требуемые размеры `AppIcon.appiconset`, отображается в собранном `.app` и не нарушает политику хранения оригинальных ресурсов |
...
- [ ] П. 54 — иконка исходного приложения во Flutter-проекте
...
**Последнее обновление:** 22 июля 2026 — п. 53 выполнен: локальные Retina-кадры двух холодных запусков Gaul автоматически сверяются по глобальной сцене и зоне персонажа; PNG и ASTPAK остаются вне Git.
