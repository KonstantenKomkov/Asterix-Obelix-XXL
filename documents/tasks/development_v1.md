...
**Статус:** выполняется. Базовый 3D-движок M3 завершён; следующий этап — ввод для gameplay vertical slice.
...
**Последнее обновление:** 21 июля 2026 — п. 31 выполнен и M3 закрыт: runtime-панель без пересборки переключает Metal wireframe, world-space collision overlay и object-ID раскраску; trigger/navmesh slots явно показывают отсутствие данных, HUD публикует CPU/GPU/frame/memory и scene counters.
