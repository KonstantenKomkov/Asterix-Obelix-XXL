...
|---:|---|---|---|---|---|
| 83 | **Провести сквозной runtime-аудит и правильно привязать все анимации управляемого Астерикса:** сопоставить каждое фактически достижимое gameplay-действие и переход с authored clip оригинала, устранить общие состояния и fallback, скрывающие разные движения | M4 fidelity | P0 | L | После п. 57–69 и фактических замечаний по бегу/двойному прыжку; обязательны: полный список достижимых состояний и переходов (idle, старт/остановка и направления движения, бег, оба прыжка/сальто, вершина/падение/приземление, атаки и combo, получение урона, смерть, interaction и доступные traversal-состояния); покадровая сверка каждого binding с локальным оригиналом и исходным hero dictionary; отдельное data-driven runtime-состояние для каждого визуально различимого действия без clip IDs в renderer; корректные loop/clamp, cadence, blend, root motion и animation events; машинная проверка, что у каждого достижимого gameplay-перехода есть ровно один совместимый binding и ни один semantic clip не подменяет другое действие; fresh ASTPAK, cold-start прохождение representative sequence и зафиксированная visual acceptance должны подтверждать результат без ручного переключения диагностических clips |
...
- [ ] П. 83 — сквозной runtime-аудит и правильная привязка всех анимаций управляемого Астерикса
...
**Последнее обновление:** 23 июля 2026 — добавлен п. 83: предыдущие проверки полноты каталога и graph не гарантировали, что фактически достижимые gameplay-состояния renderer различают визуально и связывают с правильными authored clips. После последовательных расхождений в беге и сальто двойного прыжка требуется единый runtime-аудит всех движений, действий и переходов Астерикса с покадровой сверкой оригинала, fresh-ASTPAK/cold-start приёмкой и запретом semantic/fallback-подмен.
