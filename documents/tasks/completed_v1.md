...
Player snapshot теперь публикует фактическую горизонтальную скорость, последнее
направление движения, независимые таймеры idle/run clips и плавный locomotion
blend длительностью 0,12 с. Run phase масштабируется относительно `run_speed`,
поэтому playback следует разгону и торможению капсулы. Metal renderer смешивает
локальные joint transforms idle/run до построения полной 58-bone palette и
поворачивает итоговую модель по вектору движения; возврат в idle сохраняет
непрерывную позу.

Fixed-tick XCTest покрывает `idle→run→idle`, достижение полной скорости, связь
скорости с animation phase, направление +X, плавные веса входа/выхода и
сохранение последнего yaw после остановки. Visual pose regression на
синтетическом skinned vertex фиксирует промежуточные силуэты обоих переходов и
не допускает скачка palette. Прошли 34 native XCTest, `make check` (56
Flutter-тестов), macOS debug build, diff review и resource policy; игровые
