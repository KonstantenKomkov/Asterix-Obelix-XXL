...
**Выполнено:** 22 июля 2026. **Исправлено после runtime-ревью:** 22 июля 2026.

Первоначальная реализация ошибочно назначала состоянию `run` файл `0055`,
содержащий только две малоподвижные позы, и не включала начальный keyframe
каждого RenderWare bone track. В результате state и направление менялись, но
видимого бегового цикла не было. После проверки локально извлечённого каталога
подключён многокадровый locomotion clip `0054`; reconstruction теперь сохраняет
позу `time=0`, а runtime отклоняет кандидат на run, если заметное движение есть
менее чем в 20 из 58 tracks.
...
локальном Gaul ASTPAK: HUD переходит в `run`, а `0054` загружается как пригодный
58-bone clip. Прошли 35 native XCTest, `make check` (56 Flutter-тестов), macOS
debug build, diff review и resource policy; локальный ASTPAK и screenshots не
добавлялись в Git.
