...
Gameplay bindings находятся в versioned
`assets/animation_bindings.v1.json` и упаковываются как ресурс ASTPAK
`animation-bindings`. Схема, валидация и политика runtime-ошибок описаны в
[`animation_binding_registry.md`](../architecture/animation_binding_registry.md).
Metal renderer больше не содержит таблицу clip numbers: он разрешает actor,
skin, costume, action/event, variant и context и получает loop policy из
выбранной привязки.
